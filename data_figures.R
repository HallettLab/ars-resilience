...
source("/Users/maddy/Repositories/greatbasinresilience/ars-resilience/data_analysis_paper1.R")
...
AG_localhet <- filter(AG_localhet,!is.na(potential))
PG_localhet <- filter(PG_localhet,!is.na(potential))
S_no_localhet <- filter(S_no_localhet,!is.na(potential))
S_re_localhet <- filter(S_re_localhet,!is.na(potential))
...
# Pasture level plots arranged with functional groups side by side ----
...
PGnc1 <- ggplot(data = functionalcover_pasture_plus[functionalcover_pasture_plus$FuncGroup=="PG"&functionalcover_pasture_plus$Crested==F,],aes(x=FireHistory,y=cover+0.01)) +
  ...
  labs(x = "Fire History",y = element_blank()) +
  geom_jitter(aes(fill=cover,shape=Crested),size=3,colour = "black",show.legend = F) +
  scale_fill_gradientn(colours=pgcolors,limits=pglimits) +
  scale_shape_manual(values=c(21,24))
SRnc1 <- ggplot(data = subset(functionalcover_shrubcats_pasture_plus,Resprout==1&Crested==F),aes(x=FireHistory,y=cover+0.01)) +
  ...
  labs(x = "Fire History",y = element_blank()) +
  geom_jitter(aes(fill=cover,shape=Crested),size=3,colour = "black",show.legend = F) +
  scale_fill_gradientn(colours=shrubcolors,limits=srlimits) +
  scale_shape_manual(values=c(21,24))
...
tfireplots <- plot_grid(AG_tfire,PG_tfire,SR_tfire,SN_tfire,
                        nrow=2,labels="AUTO",
                        label_fontface="plain",label_size=10)
